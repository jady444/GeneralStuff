...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "web" do |web|
    web.vm.box = "bento/ubuntu-17.04"
	web.vm.network :private_network, ip: "10.0.0.10"
	web.vm.hostname = "web"
	web.vm.define "UbuntuWEB"
	web.vm.provider :virtualbox do |vb1|
		vb1.name = "Web"
		vb1.customize ["modifyvm", :id, "--natdnshostresolver1", "on"]
		vb1.customize ["modifyvm", :id, "--natdnsproxy1", "on"]
	end
  end
  
  config.vm.define "db" do |db|
    db.vm.box = "bento/ubuntu-17.04"
	db.vm.network :private_network, ip: "10.0.0.11"
	db.vm.hostname = "db"
	db.vm.define "UbuntuDB"
	db.vm.provider :virtualbox do |vb2|
		vb2.name = "DB"
